...
def embed_texts(client: OpenAI, texts: List[str], input_type: str) -> np.ndarray:
    """
    ...
    for i in range(0, len(texts), batch_size):
        ...
        resp = client.embeddings.create(model=EMBEDDING_MODEL, input=batch, extra_body={"input_type": input_type,"truncate": "END"})
...
def build_index(client: OpenAI) -> Dict[str, Any]:
    ...
    embeddings = embed_texts(client, texts, input_type="passage")
...
def retrieve_top_k(client: OpenAI, index: Dict[str, Any], query: str, k: int = 5) -> List[Dict[str, Any]]:
    q_vec = embed_texts(client, [query], input_type="passage")[0]  # normalized
